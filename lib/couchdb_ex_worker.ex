defmodule CouchDBEx.Worker do
  ...
  def handle_call({:db_exists?, db_name}, _from, state) do
    with {:ok, resp} <- HTTPClient.head("#{state[:hostname]}:#{state[:port]}/#{db_name}") do
      {:reply, {:ok, resp.status_code == 200}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  end
  ...
  def handle_call({:db_create, db_name, opts}, _from, state) do
    ...
    with {:ok, resp} <-
         ...
         %{"ok" => true} <- resp.body |> Poison.decode!() do
      {:reply, :ok, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  ...
  @impl true
  def handle_call({:uuid_get, count}, _from, state),
    do: {:reply, uuid_get_impl(count, state), state}

  @impl true
  def handle_call(
        {:document_insert_id, document, database, id},
        _from,
        state
      ) do
    ...
    with {:ok, resp} <-
         ...
         %{"ok" => true} = rp <- resp.body |> Poison.decode!() do
      {:reply, {:ok, rp}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  ...
  @impl true
  def handle_call(
        {:document_insert, document_or_documents, database},
        _from,
        state
      )
    ...
    if is_list(document_or_documents) do
      ...
      with {:ok, resp} <-
           ...
           docs <- resp.body |> Poison.decode!() do
        {:reply, {:ok, docs}, state}
      else
        e -> {:reply, transform_error(e), state}
      end
  ...
  def handle_call({:document_get, id, database, opts}, _from, state) do
    with {:ok, resp} <-
           HTTPClient.get(
             "#{state[:hostname]}:#{state[:port]}/#{database}/#{id}",
             # This header is required, because if we request attachments, it'll return JSON as binary data and cause an error
  ...
  def handle_call({:document_find, selector, database, opts}, _from, state)
      when is_map(selector) do
    final_opts =
      opts
      # Transform options into a map
      |> Enum.into(%{})
    ...
    with {:ok, resp} <-
         ...
         %{"docs" => _docs} = json_res <- resp.body |> Poison.decode!() do
      {:reply, {:ok, json_res}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  end
  ...
  """
  def handle_call(
        {:document_delete, id_rev, database},
        from,
        state
      )
    ...
    if is_list(id_rev) do
      final_id_rev =
        Enum.map(id_rev, fn {id, rev} -> %{:_id => id, :_rev => rev, :_deleted => true} end)
      ...
      with {:ok, resp} <-
           ...
           %{"ok" => _ok} = json_res <- resp.body |> Poison.decode!() do
        {:reply, {:ok, json_res}, state}
      else
        e -> {:reply, transform_error(e), state}
      end
  ...
  @impl true
  def handle_call(
        {:attachment_upload, database, id, rev, {attachment_name, attachment_bindata}, opts},
        _from,
        state
      )
    ...
    with {:ok, resp} <-
           HTTPClient.put(
             ...
             attachment_bindata,
             if(Keyword.has_key?(opts, :content_type),
               do: [{"Content-Type", opts[:content_type]}],
               else: []
             ),
             params: [rev: rev]
           ),
  ...
  @impl true
  def handle_call(
        {:attachment_delete, database, id, rev, attachment_name},
        _from,
        state
      ) do
    with {:ok, resp} <-
  ...
  @impl true
  def handle_call(
        {:index_create, index, database, opts},
        _from,
        state
      )
    ...
    final_index =
      if is_list(index) do
        # Is index is a list, consider it a list of indexing fields
        %{fields: index}
      else
        index
      end
    ...
    with {:ok, resp} <-
         ...
         %{"result" => "created"} = json_res <- resp.body |> Poison.decode!() do
      {:reply, {:ok, json_res}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  ...
  def handle_call({:index_list, database}, _from, state) do
    with {:ok, resp} <- HTTPClient.get("#{state[:hostname]}:#{state[:port]}/#{database}/_index"),
         %{"indexes" => indexes, "total_rows" => total} <- resp.body |> Poison.decode!() do
      {:reply, {:ok, indexes, total}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  end
  ...
  def handle_call({:view_exec, ddoc, view, db, opts}, _from, state) do
    with {:ok, resp} <-
         ...
         %{"total_rows" => _} = json_resp <- resp.body |> Poison.decode!() do
      {:reply, {:ok, json_resp}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  ...
  * `cancel` - cancel the continuous replication (note that cancel request should be identical to the
  ...
  def handle_call({:replicate, source, target, opts}, _from, state) do
    ...
    with {:ok, resp} <-
         ...
         %{"ok" => true} = json_resp <- resp.body |> Poison.decode!() do
      {:reply, {:ok, json_resp}, state}
    else
      e -> {:reply, transform_error(e), state}
    end
  ...
  defp uuid_get_impl(count, state) do
    with {:ok, resp} <-
           HTTPClient.get("#{state[:hostname]}:#{state[:port]}/_uuids", [], params: [count: count]),
         %{"uuids" => uuids} <- resp.body |> Poison.decode!() do
      {:ok, uuids}
    else
      e -> transform_error(e)
    end
